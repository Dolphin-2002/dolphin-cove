...
// Get live counts for notification & message badges
$_headerUnreadNotifs = 0;
$_headerUnreadMsgs = 0;
if (isLoggedIn()) {
    $__uid = getCurrentUserId();
    $__db = getDB();
    // Unread notifications
    $__nStmt = $__db->prepare("SELECT COUNT(*) AS cnt FROM notifications WHERE user_id = ? AND is_read = 0");
    $__nStmt->bind_param("i", $__uid);
    $__nStmt->execute();
    $_headerUnreadNotifs = (int)$__nStmt->get_result()->fetch_assoc()['cnt'];
    $__nStmt->close();
    // Unread messages
    $__mStmt = $__db->prepare("SELECT COUNT(*) AS cnt FROM messages WHERE receiver_id = ? AND is_read = 0");
    $__mStmt->bind_param("i", $__uid);
    $__mStmt->execute();
    $_headerUnreadMsgs = (int)$__mStmt->get_result()->fetch_assoc()['cnt'];
    $__mStmt->close();
}
?>
<!DOCTYPE html>
            ...
            <nav class="header-nav">
                <?php if (isLoggedIn()): ?>
                    ...
                    <a href="<?= SITE_URL ?>/messages.php" class="nav-icon-btn" title="Messages">
                        ...
                        <?php if ($_headerUnreadMsgs > 0): ?>
                        <span class="header-badge"><?= $_headerUnreadMsgs > 99 ? '99+' : $_headerUnreadMsgs ?></span>
                        <?php endif; ?>
                    </a>
                    <a href="<?= SITE_URL ?>/notifications.php" class="nav-icon-btn" title="Notifications">
                        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>
                        <?php if ($_headerUnreadNotifs > 0): ?>
                        <span class="header-badge"><?= $_headerUnreadNotifs > 99 ? '99+' : $_headerUnreadNotifs ?></span>
                        <?php endif; ?>
                    </a>
